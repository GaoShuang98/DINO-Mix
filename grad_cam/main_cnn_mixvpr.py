...
def main():
    ...
    resume_dir = r'D:\python_code\MixVPR(hgs)\LOGS\resnet50\resnet50_MixVPR_4096_channels(1024)_rows(4)_epoch(05)_step(4692)_R1[0.9135]_R5[0.9728].ckpt'
    model_net = VPRModel(
        # ---- Encoder
...
def grad_cam_batch(batch_dir, model, target_layers):
    ...
    for img_name in img_list:
        ...
        plt.axis('off')
        plt.subplots_adjust(left=0, right=1, bottom=0, top=1)
        plt.savefig(os.path.join(r'D:\python_code\MixVPR(hgs)\grad_cam(热力图可视化)\热力图结果-不同方法\MixVPR', 'MixVPR'+img_name))
        plt.show()
        plt.close()
