...
def test(args, eval_ds, model, test_method="hard_resize", pca=None, flops_times=True):
    # 先检查并确保模型在正确的设备上
    model = model.to(args.device)
    # 如果模型被DataParallel封装，访问底层模块
    ...
    if flops_times:
        # 使用thop计算FLOPs
        dummy_input = torch.randn(args.infer_batch_size, 3, args.resize[0], args.resize[1]).to(args.device)
        flops, params = thop.profile(model, inputs=(dummy_input,))
        # 测量推理时间
        start_time = time.time()
        # 在循环外运行模型以获取平均推理时间，避免结果偏差
        with torch.no_grad():
            _ = model(dummy_input)
        end_time = time.time()
        inference_time = end_time - start_time
    # ------------------------------------------------------------------------------------------------------------------]
    return recalls, recalls_str, flops, inference_time
